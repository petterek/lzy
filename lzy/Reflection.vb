            ...
            If _allTypes Is Nothing Then
                SyncLock _PadLock
                    If _allTypes Is Nothing Then
                        ...
                        For Each a In AppContext.CurrentDomain.GetAssemblies()
                        ...
                        Next
                        Try
                            Dim fileInfos As FileInfo() = New DirectoryInfo(AppContext.BaseDirectory).GetFiles("*.dll", SearchOption.AllDirectories)
                            ...
                            Next
                        Catch ex As Exception
                            Throw New exception("DLL Loading", ex)
                        End Try
                        For Each assembly In loaded
                            Try
                                ...
                                For Each type In getTypes
                                    Try
                                        If type.GetTypeInfo.IsClass AndAlso Not type.GetTypeInfo.IsAbstract Then
                                            allTypesTemp.Add(type)
                                            _guidmap.Add(type.GetTypeInfo.GUID, type)
                                        End If
                                    Catch ex As Exception
                                        Throw New Exception("Add type" & type.FullName, ex)
                                    End Try
                                Next
